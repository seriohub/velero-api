...
class AuthenticationService:
    ...
    def __init__(self):
        self.token_access_expire = config_app.get_security_token_expiration()
        self.token_refresh_expires_days = config_app.get_security_token_refresh_expiration()

        self.response_data = ResponseData()

    def __create_token(self, username, userid, db: Session, only_access=True):
        logger.debug(f"__create_token username:{username}")
    ...
    def login(self, username, password, db: Session):
        ...
        if len(username) > 1 and len(password) > 1:
            ...
            # )
            # logger.debug(f"user name: {user.username} id: {user.id}")
            # # Update the user's refresh token in the database
            # add_refresh_token(refresh_token=refresh_token,
            #                   user_id=user.id,
            #                   db=db)
